...
const W_5ML = 65;    // g
const W_10ML = 120;  // g
const W_PACK_BASE = 80; // g (caja + burbuja común del pedido)

type CartWeightItem = { ml?: number; qty: number };
...
/** Talla por peso: XS ≤0,5 kg, S ≤3 kg, M ≤6 kg, L >6 kg */
...
interface CartItem {
  ...
  brand?: string;
  name: string;
  ml?: number; // 3 | 5 | 10 para decants
}

export default function CheckoutPage() {
  const router = useRouter();
  const items = useCart((s) => s.items) as unknown as CartItem[];
  ...
  useEffect(() => {
    ...
    const grams = decantsWeight(items.map(i => ({ ml: i.ml, qty: i.qty })));
    ...
    const fee = calcShipping({
      ...
      tramo,
      size,
      subtotal,           // aplica envío gratis si supera umbral definido en ShippingMarquee
      table: TARIFF_TABLE,
    });
    ...
    setShippingReason(
      fee === 0
        ...
        : `Domicilio · ${tramoLabel(tramo)} · ${(grams / 1000).toFixed(2)} kg · talla ${size}`
    );
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (out2.redirectUrl) { window.location.href = out2.redirectUrl; return; }
          ...
          <form onSubmit={submit} className="space-y-6">
            <section className="rounded-2xl border border-slate-200 p-4 md:p-6">
              ...
              <div className="space-y-2">
                {([{ id: "WEBPAY", label: "Webpay | Débito y Crédito" }] as const).map((opt) => (
                  <label
                    key={opt.id}
                    className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer ${paymentMethod === opt.id ? "border-blue-500 bg-blue-50" : "border-slate-300"}`}
                  >
                    <input
                      type="radio"
                      name="paymentMethod"
            ...
            <section className="rounded-2xl border border-slate-200 p-4 md:p-6 space-y-4">
              ...
              <button
                type="submit"
                disabled={disabled}
                className={`w-full md:w-auto px-6 py-3 rounded-2xl font-semibold text-white ${disabled ? "bg-slate-400" : "bg-blue-600 hover:bg-blue-700"}`}
              >
