...
const isCategoria = (x: unknown): x is Categoria => CATS.includes(String(x) as Categoria);

// Género
const GENEROS = ["HOMBRE", "MUJER", "UNISEX"] as const;
type Genero = (typeof GENEROS)[number];
const isGenero = (x: unknown): x is Genero => GENEROS.includes(String(x).toUpperCase() as Genero);
...
export default function Client() {
  ...
  const [precioInput, setPrecioInput] = useState<string>(""); // controlado
  const [mlInput, setMlInput] = useState<string>("");         // controlado
      ...
      <div className="max-w-2xl mx-auto rounded-2xl border border-white/20 bg-white/10 backdrop-blur p-6">
        ...
        <form onSubmit={onSubmit} className="grid gap-4">
          ...
          <div className="text-xs text-white/70">
            Previsualización precio: <span>{money(Number(precioInput || 0))}</span>
          </div>
        </form>
      </div>
    </main>
