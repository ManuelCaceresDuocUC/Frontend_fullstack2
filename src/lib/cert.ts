...
export function loadP12Pem(): { keyPem: string; certPem: string } {
  ...
  const certObj = certBags[0]?.cert as forge.pki.Certificate | undefined;
  if (!certObj) throw new Error("No se encontró certificado en el P12");

  const keyObj =
    (keyBagsPkcs8[0]?.key as forge.pki.PrivateKey | undefined) ??
    (keyBagsPkcs1[0]?.key as forge.pki.PrivateKey | undefined);
