...
export function marginForMl(ml: number): number {
  if (ml === 3) return Number(process.env.M3 ?? 1.55);
  if (ml === 5) return Number(process.env.M5 ?? 1.45);
  if (ml === 10) return Number(process.env.M10 ?? 1.35);
  return Number(process.env.M5 ?? 1.45);
}
