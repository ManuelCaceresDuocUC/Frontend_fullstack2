...
type OrderView = { id:string; total:number; status:OrderStatus; items:OrderItemView[] };
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        setOrder(o);
        // si aún está PENDING, reintenta un par de veces por si hay delay entre Webpay y DB
        if (o.status === "PENDING" && tries < 6 && !stop) {
          tries++;
